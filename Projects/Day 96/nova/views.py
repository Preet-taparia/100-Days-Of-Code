...
def followers(request, pk):
    if request.user.is_authenticated:
        profiles = Profile.objects.get(user_id=request.user)
...
def delete_post(request, pk):
    if request.user.is_authenticated:
        ...
        if request.user.username == post.user.username:
            post.delete()
            messages.success(request, ("You Have Succcessfully deleted the post"))
            return redirect(request.META.get("HTTP_REFERER") )
        else:
            messages.success(request, ("You Cannot Do This Action..."))
            return redirect('home')
...
def search_post(request):
    if request.method == 'POST':
        ...
        searched = Posts.objects.filter(body__contains = search)
